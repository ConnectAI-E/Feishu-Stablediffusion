...
class GenerateConfig:
    def __init__(self):
        self.prompt = '1girl, beautiful, cute, long hair, original, pink hair, solo, white background, masterpiece, best quality'
        self.negative_prompt = 'lowres, bad anatomy, bad hands, text, error, missing fingers, extra digit, fewer digits, cropped, worst quality, low quality, normal quality, jpeg artifacts, signature, watermark, username, blurry, artist name'
        self.model = 'SD'
    ...
    def update_image_configuration(self, config, image_cfg):
        ...
        if 'override_settings_restore_afterwards' in config:
            image_cfg.set_override_settings_restore_afterwards(config['override_settings_restore_afterwards'])
