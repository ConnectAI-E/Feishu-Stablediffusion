...
class CommandHandler:
    ...
    def handle_command(self, event):
        ...
        if command.startswith("/help"):
            self.message_sender.send_card(back_id, image_configuration.help())
            ...
            self.message_sender.send_card(back_id, image_configuration.list_models(model_names))
            ...
            self.message_sender.send_card(back_id, image_configuration.queue(queue))
            app_logger.info(f"/queue")
        else:
            self.message_sender.send_text_message(back_id, "unknown command")
